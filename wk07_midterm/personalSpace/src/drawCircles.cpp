...
void drawCircles::setup(ofFbo & _fbo){
    ...
    clearFbo(maskFbo);
    //comment out here if you dont want particles
    //setupParticles ();
   // myFont.loadFont("blender.ttf", 10);
    
    // temporary ofPath to draw circle to get the Mesh to store into Vbo of graphi
    ofPath circlePath;
...
void drawCircles::drawViaFBO () {
    ...
    for (int i=rings *2 ; i >1 ; i--){ //int i=rings* 2 + 1
        ...
        //you need create a for loop to count no of blobs then add draw code here.
    }
}
