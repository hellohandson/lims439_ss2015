...
//------------------------------------------------------------------
void dotParticle::setMode(particleMode newMode){
    mode = newMode;
}

//------------------------------------------------------------------
void dotParticle::setAttractPoints( vector <ofPoint> * attract ){
    attractPoints = attract;
}

//------------------------------------------------------------------
void dotParticle::reset(){
...
void dotParticle::update(){
    ...
    if( mode == PARTICLE_MODE_ATTRACT ){
        ...
        for(unsigned int i = 0; i < attractPoints->size(); i++){
            ofPoint attractPt = attractPoints->at(i)-pos;
            frc = attractPt ;
            // cout << i << " p:" << frc << "\n" ;
            ...
            vel *= drag;
            if( dist < repelSize ){
                vel += -frc * 0.9; //notice the frc is negative
            }else{
                //if the particles are not close to us, lets add a little bit of random movement using noise. this is where uniqueVal comes in handy.
                frc.x = ofSignedNoise(uniqueVal, pos.y * 0.05, ofGetElapsedTimef()*0.5);
                frc.y = ofSignedNoise(uniqueVal, pos.x * 0.05, ofGetElapsedTimef()*0.5);
                vel += frc * 0.54;
            }
        }
...
void dotParticle::draw(){
    ...
    if( mode == PARTICLE_MODE_ATTRACT ){
        ...
        //color here. i think you can only use repel n nearest point modes
        ofSetColor(208, 0, 0);
    }
    else if( mode == PARTICLE_MODE_NOISE ){
