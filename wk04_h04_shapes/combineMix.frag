...
void main(){
    ...
    vec3 color2 = vec3(.1, 0.9, 0.2);
  vec3 color = vec3 (0.);
  
  float pct1 = drawPoly(gl_FragCoord.xy, vec2(u_resolution.x/2.,60.), 20. , 8 );
  float pct2 = drawCircleRings(gl_FragCoord.xy, u_resolution.xy/2., 125. , 5);
  float pct3 = drawPoly(gl_FragCoord.xy, vec2(u_resolution.x/2.,30.), 30. , 5 );

  pct3 = drawCirclePixelOutline(gl_FragCoord.xy, u_resolution.xy/2., 200. , 8. );
  pct3 = drawCircleRings(gl_FragCoord.xy, u_resolution.xy/2., 250. , 20);

  float pct = min(pct2 , pct3);

  color = vec3 (1.-  pct3  * pct , 1.-pct3 , 1.-pct  );
