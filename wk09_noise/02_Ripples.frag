...
void main( void ) {
    ...
    vec2 position = ( gl_FragCoord.xy / vec2(u_resolution.x*1.9, u_resolution.y*1.5));
    float scale = 3.0;
    ...
    x -= midpoint.x;
    y += midpoint.y;
    float b = snoise(vec3(x, y, u_time / 3.333)) * 0.5 + 0.5;
    
    n = snoise(vec3(pow(b+1.0, r)*scale, x*2.0, u_time / 2.718281828459));
    q = snoise(vec3(pow(b+1.0, r)*scale, theta, u_time / 1.618033988749));
